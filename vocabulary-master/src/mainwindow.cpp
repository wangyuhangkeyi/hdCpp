...
void MainWindow::on_pushButton_6_clicked()
{
    ...
    for(int i=0;i<incount;i++)  //new 判断英文
    {
        if(!(((input.at(i)>='a')&&(input.at(i)<='z'))||((input.at(i)>='A')&&(input.at(i)<='Z'))))
         {   QMessageBox::warning(this,"查询失败","你输入的不是英文单词！",QMessageBox::Yes);
             return;
         }
...
void MainWindow::on_pushButton_7_clicked()//new  汉译英
{
    QString input = ui->lineEdit->text();
    if(input.isEmpty())      //判断是否有输入查询内容
    {
    ...
    {
        if((((input.at(i)>='a')&&(input.at(i)<='z'))||((input.at(i)>='A')&&(input.at(i)<='Z'))))
         {   QMessageBox::warning(this,"查询失败","你输入的不是中文！",QMessageBox::Yes);
             return;
         }

    }
    QSqlQuery query;
    query.prepare("select * from vocabulary where definition LIKE :definition");//搜索以:definition开头的所有单词，全字匹配是where definition = :definition
    query.bindValue(":definition","%"+ui->lineEdit->text()+"%");
    query.exec();
    if(!query.next())
    {
        QMessageBox::warning(this,"查询失败","没有查找到该单词！",QMessageBox::Yes);
